function LTC3851A_ex5_bode_response(example)
% -----------------------------------------------------------------
% LTC3851A_ex5_bode_response.m
%
...
% the LTSpice LTC3851A_ex5_bode.asc circuit. The circuit writes
% the results to its .log file. That log file is then renamed
% to something like LTC3851A_ex5_bode1.txt so that LTspice does
...
% ----------
%
% Example#5 uses the same setup as Example#4
%  * Inductor DCR sensing attenuation As = 0.577
%  * Rsns = Rdcr x As = 1.5mOhm
% But changed the compensation network to move the zero higher
% and allowed gain at fsw.
%
% Example #6
% ----------
%
% Move the zero to lower frequency to show how that changes the
% transient response and increases the phase margin.
%
% -----------------------------------------------------------------
...
% -----------------------------------------------------------------
%
% LTC3851A_ex5_bode1.txt contains the measurements at all
% frequency points (including 100 and 300Hz).
%
% Select example 5 or 6 to determine the file to read and Rsns value
if (nargin == 0)
	% Default to example 5
	example = 5; 
end

if (example == 5) 
	filename = 'LTC3851A_ex5_bode1.txt';
	fprintf('Example#5: Rsns = 1.5mOhm and high cross-over frequency\n')
elseif (example == 6)
	filename = 'LTC3851A_ex5_bode2.txt';
	fprintf('Example#6: Move the compensation zero down to 10kHz\n')
else
