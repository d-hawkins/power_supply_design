function LTC3851A_ex3_bode_response(example)
% -----------------------------------------------------------------
% LTC3851A_ex3_bode_response.m
%
...
% the LTSpice LTC3851A_ex3_bode.asc circuit. The circuit writes
% the results to its .log file. That log file is then renamed
% to something like LTC3851A_ex3_bode1.txt so that LTspice does
...
% ----------
%
% Relative to Example #2, the design was changed to use inductor
% DCR current-sensing, with the DCR sensing network attenuation,
% with the same compensation network as #2.
%
% Example#2 cross-over/margin = 42.77 kHz/48.25-degrees.
%
% a) 'LTC3851A_ex3_bode1.txt' (Example#3)
%
%     * Inductor DCR sensing attenuation As = 0.769
%     * Rsns = Rdcr x As = 2.0mOhm to match the current sense 
%      resistor in Example #2.
%     * Cross-over frequency/phase-margin = 43.03 kHz/48.46 degrees
%       (virtually identical to Example#2)
%
%,   The low-frequency response is a little different than Ex2;
%    the DCR version has higher gain. This is likely an artifact
%    of the RC sensing network. Since the control-to-output gain
%    is higher at low frequencies, the DC regulation is better.
%
% b) 'LTC3851A_ex3_bode2.txt' (Example#4)
%
%     * Inductor DCR sensing attenuation As = 0.577
%     * Rsns = Rdcr x As = 1.5mOhm (0.75 of Example#2)
%     * Cross-over frequency/phase-margin = 55.60 kHz/49.32 degrees
%
%    The increase in cross-over frequency was due to the increase
%    in mid-band gain. The phase margin is essentially unchanged.
%
% -----------------------------------------------------------------
...
% -----------------------------------------------------------------
%
% LTC3851A_ex3_bode1.txt contains the measurements at all
% frequency points (including 100 and 300Hz).
%

% Select example 3 or 4 to determine the file to read and Rsns value
if (nargin == 0)
	% Default to example 3
	example = 3; 
end

if (example == 3) 
	filename = 'LTC3851A_ex3_bode1.txt';
	fprintf('Example#3: Inductor DCR sensing with Rsns = 2.0mOhm\n')
elseif (example == 4)
	filename = 'LTC3851A_ex3_bode2.txt';
	fprintf('Example#4: Inductor DCR sensing with Rsns = 1.5mOhm\n')
else
